...
def split_and_pad(input_data, split_sizes,num_local_experts, d_model, ep_size):
    ...
    padded_data = padded_data.reshape(ep_size, num_local_experts, padded_data.size(1), d_model)
    return padded_data

def slicing_zero(input_data, split_sizes,num_local_experts, d_model,ep_size):
    ...
    result_tensor = torch.cat((torch.tensor([0]), result_tensor[:-1]), dim=0).reshape(-1,ep_size)
...
def zeropad_output(input_data, split_sizes,num_local_experts, d_model, capacity, ep_size):
    result_2d = []
...
class MOELayer(Base):
    ...
    def forward(self, layer_count, mask, batch, new_centroids, *input: Tensor, **kwargs: Any) -> Tensor:
        ...
        non_zero_dispatched_input = slicing_zero(dispatched_input.reshape(-1,d_model), self.exp_counts, self.num_local_experts, d_model, self.ep_size).to(dispatched_input.device).clone()
        summed_tensor = self.exp_counts.view(self.ep_size, self.num_local_experts).sum(dim=1).clone()
        output_lists = [tensor[device_number*self.num_local_experts:device_number*self.num_local_experts+self.num_local_experts].tolist() for tensor in gathered_lists]
        ...
        non_zero_output = torch.empty((int(output_tensor_sum.sum(dim=0)),d_model),device = dispatched_input.device).clone()
        ...
        dispatched_input = split_and_pad(dispatched_input,output_tensor,self.num_local_experts,d_model,self.ep_size).to(dispatched_input.device)
        expert_output = self.experts(dispatched_input)
        expert_output = slicing_zero(expert_output.reshape(-1,d_model), output_tensor, self.num_local_experts, d_model, self.ep_size).to(expert_output.device).clone()
        ...
        combined_output = zeropad_output(expert_output, self.exp_counts,self.num_local_experts ,d_model, C, self.ep_size)
