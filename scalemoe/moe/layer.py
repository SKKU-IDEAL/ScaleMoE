...
class MoE(torch.nn.Module):
    ...
    def forward(self,hidden_states, layer_count, mask, batch, used_token=None, new_centroids=None):
        """ MoE forward
        ...
        """
        # hansol
        # output, counts, expert_layer = self.deepspeed_moe(layer_count, mask, batch, hidden_states, used_token)
        output = self.deepspeed_moe(layer_count, mask, batch, new_centroids, hidden_states, used_token)
        # print("output : ", output)
        ...
        # hansol
        # return output, self.deepspeed_moe.l_aux, self.deepspeed_moe.exp_counts, self.deepspeed_moe.mask1, self.deepspeed_moe.result
        return output, self.deepspeed_moe.exp_counts, self.deepspeed_moe.mask1, self.deepspeed_moe.result
