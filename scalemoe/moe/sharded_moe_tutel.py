...
class _AllToAll_MOE(torch.autograd.Function):
    ...
    def forward(ctx: Any,
        ...
        with nvtx.annotate("All to All single in AllToAll_MOE", color="blue"):
            dist.all_to_all_single(output, input, output_slices, input_slices, group=group)
        #print("all to all finished")
        return output
...
def top1gating(layer_count,
    ...
    if device_number == 0:
        tensor[:512, 0] = 1
        tensor[512:1024, 1] = 1
        tensor[1024:1536, 2] = 1
        tensor[1536:, 3] =1
    if device_number == 1:
        tensor[:512, 4] = 1
        tensor[512:1024, 5] = 1
        tensor[1024:1536, 6] = 1
        tensor[1536:, 7] =1
    gates = tensor
    ...
    # hansol
    # print(indices1_s.size())
    # print(mask.size())
    # print(mask)
    if mask is not None:
        reshaped_mask = mask.reshape(-1)

        # print('here mask ', reshaped_mask, reshaped_mask.size())
    ...
    # print("used_token : ", used_token.squeeze(1).squeeze(1).reshape(-1).size())
    ...
    if use_tutel:
        ...
        #if device_number == 0 :
            #print("gate1_s: ", gates1_s, gates1_s.size())
            #print("l_aux: ", l_aux, l_aux.size())
        #print("num_experts: ", num_experts)
           # print("mask1: ", mask1, mask1.size())
            #print("locations1: ", locations1, locations1.size())
            #print("locations1_s: ", locations1_s, locations1_s.size())
            #print("indices1_s: ", indices1_s, indices1_s.size())
    ...
    # print("locations1_sc ",locations1_sc.size())
    combine_weights = einsum("se,sc->sec", gates, locations1_sc)
    # print(gates.size(), locations1_sc.size())
...
def split_and_pad(input_data, split_sizes,num_local_experts, d_model):
    ...
    # 8행 5열의 배열 생성
    for i in range(8):
        start_index = i * 4  # 각 행의 시작 인덱스
        row = split_indices[start_index:start_index + 5]  # 5개의 요소 선택
    ...
    padded_data = padded_data.reshape(num_local_experts, -1, padded_data.size(1), d_model).transpose(0,1).reshape(num_local_experts * 8,-1,d_model)
    return padded_data
...
def zeropad_output(input_data, split_sizes,num_local_experts, d_model, capacity):
    ...
    for i in range(8):
        start_index = i * 4  
        row = split_indices[start_index:start_index + 5]  
        result_2d.append(row)
    result_tensor = torch.cat(result_2d, dim=0).reshape(-1,num_local_experts+1)
    data_chunks = []
    for i in range(8):
...
class MOELayer(Base):
    ...
    def forward(self, layer_count, mask, batch, new_centroids, *input: Tensor, **kwargs: Any) -> Tensor:
        ...
        # if (self.count == 0):
        #     with open('output/pad/input.csv', 'a') as f:
        #         f.write("input : ")
        #         for i in range(input[0].size(0)):
        #             for j in range(input[0].size(1)):
        #                 f.write(str(input[0][i,j,:]))
        #                 f.write("\n")
        #     self.count = 1
        # print('input')
        # print("1", input[0].size())
        # print("2",reshaped_input.size())
        # print("input : ", input[0][:,0,:])
        # print("input size : ", input[0][:,0,:].size())
        # batch 만큼
        # print("reshaped_input : ", reshaped_input.size())
        # print("reshaped_input : ",reshaped_input[16,:])
        # reshaped_mask = mask.squeeze(1).squeeze(1)
        # input2 = input[0][~reshaped_mask]
        # print("gating network input(padding) : ", input2[0,:], input[0].size(), mask.size(), reshaped_input.size(), input2.size())
        # with open('output/pad/gating_network_padding_input.csv', 'a') as f:
        #     for i in range(input2.size(0)):
        #         f.write("input : ")
        #         f.write(str(input2[i,:]))
        #         f.write("\n")
        if self.use_tutel:
            # hansol
            ...
            with nvtx.annotate("All gather", color="red"):
                device_number = dist.get_rank()
                exp_counts_gpu = self.exp_counts.to(reshaped_input.device)
        ...
        '''
        if dist.get_rank() == 0:
            print(self.exp_counts, layer_count, dist.get_rank())
        
        today = datetime.today().strftime("%m%d")
        folder_name = str('/home/skku/deepspeed/BERT/output/'+today)
        with open(f'{folder_name}/exp_count_{dist.get_rank()}.csv', 'a') as f:
            for arr in self.exp_counts:
                f.write(f'{arr}')
                f.write(',')
            f.write('\n')
        ''' 
        ...
        with nvtx.annotate("Computation1", color="white"):
            non_zero_dispatched_input = dispatched_input[~(dispatched_input == 0).all(axis=1)]
            summed_tensor = self.exp_counts.view(8, 4).sum(dim=1)
        with nvtx.annotate("output_lists", color="white"):
            output_lists = [tensor[device_number*4:device_number*4+4].tolist() for tensor in gathered_lists]
        with nvtx.annotate("input_slices", color="white"):
            input_slices = summed_tensor.tolist()
        with nvtx.annotate("output_tensor", color="white"):
            output_tensor = torch.tensor(output_lists)
        with nvtx.annotate("output_tensor_sum", color="white"):
            output_tensor_sum = output_tensor.sum(dim=1)
        with nvtx.annotate("non_zero_output", color="white"):
            non_zero_output = torch.empty((int(output_tensor_sum.sum(dim=0)),768),device = dispatched_input.device)
        ...
        #dispatched_input = split_and_pad(dispatched_input,output_tensor,self.num_local_experts,d_model).to(dispatched_input.device)
        ...
        with nvtx.annotate("Expert1", color="blue"):
            expert_output = expert_output[~(dispatched_input.reshape(8,-1,d_model)==0).all(axis=-1)]
        '''
        if self.wall_clock_breakdown:
        ...
        with nvtx.annotate("AllToAll2", color="red"):
            #expert_output = _AllToAll_MOE.apply(self.ep_group, output_slices, input_slices, expert_output, non_zero_dispatched_input)
        ...
        with nvtx.annotate("zeropad", color="white"): 
            combined_output = zeropad_output(expert_output, self.exp_counts,self.num_local_experts ,d_model, C)
        '''
